...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node ~/.claude/get-it-done/bin/gid-tools.js summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `gid-tools milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to gid-tools:**

```bash
ARCHIVE=$(node ~/.claude/get-it-done/bin/gid-tools.js milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
```bash
INIT=$(node ~/.claude/get-it-done/bin/gid-tools.js init execute-phase "1")
```
...
```bash
node ~/.claude/get-it-done/bin/gid-tools.js commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
...
- [ ] User knows next step (/gid:new-milestone)
