<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates gid-phase-researcher, gid-planner, and gid-plan-checker agents with a revision loop (max 3 iterations).
</purpose>

<required_reading>
Read all files referenced by the invoking prompt's execution_context before starting.

@~/.claude/get-it-done/references/ui-brand.md
</required_reading>
...
```bash
INIT=$(node ~/.claude/get-it-done/bin/gid-tools.js init plan-phase "$PHASE" --include state,roadmap,requirements,context,research,verification,uat)
```
...
**If `planning_exists` is false:** Error — run `/gid:new-project` first.
...
```bash
PHASE_INFO=$(node ~/.claude/get-it-done/bin/gid-tools.js roadmap get-phase "${PHASE}")
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GID ► RESEARCHING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning researcher...
```

### Spawn gid-phase-researcher

```bash
PHASE_DESC=$(node ~/.claude/get-it-done/bin/gid-tools.js roadmap get-phase "${PHASE}" | jq -r '.section')
...
STATE_SNAP=$(node ~/.claude/get-it-done/bin/gid-tools.js state-snapshot)
...
<phase_context>
IMPORTANT: If CONTEXT.md exists below, it contains user decisions from /gid:discuss-phase.
...
```
Task(
  prompt="First, read ~/.claude/agents/gid-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
## 8. Spawn gid-planner Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GID ► PLANNING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
**Phase Context:**
IMPORTANT: If context exists below, it contains USER DECISIONS from /gid:discuss-phase.
...
<downstream_consumer>
Output consumed by /gid:execute-phase. Plans need:
...
```
Task(
  prompt="First, read ~/.claude/agents/gid-planner.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
...
## 10. Spawn gid-plan-checker Agent
...
Task(
  ...
  subagent_type="gid-plan-checker",
  model="{checker_model}",
...
- /gid:plan-phase {X} --research — re-research first
...
- [ ] gid-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] gid-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] gid-plan-checker spawned with CONTEXT.md
