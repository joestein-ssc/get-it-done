...
**Delegate ROADMAP.md and STATE.md updates to gid-tools:**

```bash
TRANSITION=$(node ~/.claude/get-it-done/bin/gid-tools.js phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `gid-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node ~/.claude/get-it-done/bin/gid-tools.js progress bar --raw)
```
...
**Use the transition result from `gid-tools phase complete`:**
...
Exit skill and invoke SlashCommand("/gid:plan-phase [X+1]")
...
**Also available:**
- `/gid:discuss-phase [X+1]` — gather context first
- `/gid:research-phase [X+1]` — investigate unknowns
- Review roadmap
...
Exit skill and invoke SlashCommand("/gid:complete-milestone {version}")
