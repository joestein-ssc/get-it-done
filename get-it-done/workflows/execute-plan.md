...
@~/.claude/get-it-done/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node ~/.claude/get-it-done/bin/gid-tools.js init execute-phase "${PHASE}" --include state,config)
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="gid-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn gid-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
See `~/.claude/get-it-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/get-it-done/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/get-it-done/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/get-it-done/templates/summary.md`.
...
Update STATE.md using gid-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node ~/.claude/get-it-done/bin/gid-tools.js state advance-plan

# Recalculate progress bar from disk state
node ~/.claude/get-it-done/bin/gid-tools.js state update-progress

# Record execution metrics
node ~/.claude/get-it-done/bin/gid-tools.js state record-metric \
...
node ~/.claude/get-it-done/bin/gid-tools.js state add-decision \
  --phase "${PHASE}" --summary "${DECISION_TEXT}" --rationale "${RATIONALE}"

# Add blockers if any found
node ~/.claude/get-it-done/bin/gid-tools.js state add-blocker "Blocker description"
```
</step>

<step name="update_session_continuity">
Update session info using gid-tools:

```bash
node ~/.claude/get-it-done/bin/gid-tools.js state record-session \
...
```bash
node ~/.claude/get-it-done/bin/gid-tools.js commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
</step>
...
```bash
node ~/.claude/get-it-done/bin/gid-tools.js commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/gid:execute-phase {phase}` + `/gid:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/gid:plan-phase {Z+1}` + `/gid:verify-work {Z}` + `/gid:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/gid:complete-milestone` + `/gid:verify-work` + `/gid:add-phase` |
