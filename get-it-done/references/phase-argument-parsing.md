...
```bash
PHASE_INFO=$(node ~/.claude/get-it-done/bin/gid-tools.js find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/get-it-done/bin/gid-tools.js roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/get-it-done/bin/gid-tools.js find-phase "${PHASE}" --raw)
```
