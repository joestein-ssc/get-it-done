...
/**
 * GID Tools — CLI utility for GID workflow operations
 *
 * Replaces repetitive inline bash patterns across ~50 GID command/workflow/agent files.
 * Centralizes: config parsing, model resolution, phase lookup, git commits, summary verification.
 *
 * Usage: node gid-tools.js <command> [args] [--raw]
 *
 * Atomic Commands:
...
const MODEL_PROFILES = {
  'gid-planner':              { quality: 'opus', balanced: 'opus',   budget: 'sonnet' },
  'gid-roadmapper':           { quality: 'opus', balanced: 'sonnet', budget: 'sonnet' },
  'gid-executor':             { quality: 'opus', balanced: 'sonnet', budget: 'sonnet' },
  'gid-phase-researcher':     { quality: 'opus', balanced: 'sonnet', budget: 'haiku' },
  'gid-project-researcher':   { quality: 'opus', balanced: 'sonnet', budget: 'haiku' },
  'gid-research-synthesizer': { quality: 'sonnet', balanced: 'sonnet', budget: 'haiku' },
  'gid-debugger':             { quality: 'opus', balanced: 'sonnet', budget: 'sonnet' },
  'gid-codebase-mapper':      { quality: 'sonnet', balanced: 'haiku', budget: 'haiku' },
  'gid-verifier':             { quality: 'sonnet', balanced: 'sonnet', budget: 'haiku' },
  'gid-plan-checker':         { quality: 'sonnet', balanced: 'sonnet', budget: 'haiku' },
  'gid-integration-checker':  { quality: 'sonnet', balanced: 'sonnet', budget: 'haiku' },
};
...
function loadConfig(cwd) {
  ...
  const defaults = {
    ...
    phase_branch_template: 'gid/phase-{phase}-{slug}',
    milestone_branch_template: 'gid/{milestone}-{slug}',
    research: true,
    plan_checker: true,
    verifier: true,
...
function cmdConfigEnsureSection(cwd, raw) {
  ...
  const braveKeyFile = path.join(homedir, '.gid', 'brave_api_key');
...
function cmdPhaseAdd(cwd, description, raw) {
  ...
  // Build phase entry
  const phaseEntry = `\n### Phase ${newPhaseNum}: ${description}\n\n**Goal:** [To be planned]\n**Depends on:** Phase ${maxPhase}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run /gid:plan-phase ${newPhaseNum} to break down)\n`;
...
function cmdPhaseInsert(cwd, afterPhase, description, raw) {
  ...
  // Build phase entry
  const phaseEntry = `\n### Phase ${decimalPhase}: ${description} (INSERTED)\n\n**Goal:** [Urgent work - to be planned]\n**Depends on:** Phase ${afterPhase}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run /gid:plan-phase ${decimalPhase} to break down)\n`;
...
function cmdScaffold(cwd, type, options, raw) {
  ...
  switch (type) {
    case 'context': {
      ...
      content = `---\nphase: "${padded}"\nname: "${name || phaseInfo?.phase_name || 'Unnamed'}"\ncreated: ${today}\n---\n\n# Phase ${phase}: ${name || phaseInfo?.phase_name || 'Unnamed'} — Context\n\n## Decisions\n\n_Decisions will be captured during /gid:discuss-phase ${phase}_\n\n## Discretion Areas\n\n_Areas where the executor can use judgment_\n\n## Deferred Ideas\n\n_Ideas to consider later_\n`;
      break;
    }
    case 'uat': {
...
function cmdInitExecutePhase(cwd, phase, includes, raw) {
  ...
  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'gid-executor'),
    verifier_model: resolveModelInternal(cwd, 'gid-verifier'),
...
function cmdInitPlanPhase(cwd, phase, includes, raw) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'gid-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'gid-planner'),
    checker_model: resolveModelInternal(cwd, 'gid-plan-checker'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'gid-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'gid-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'gid-roadmapper'),
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'gid-codebase-mapper'),
...
async function main() {
  ...
  if (!command) {
    error('Usage: gid-tools <command> [args] [--raw]\nCommands: state, resolve-model, find-phase, commit, verify-summary, verify, frontmatter, template, generate-slug, current-timestamp, list-todos, verify-path-exists, config-ensure-section, init');
  }
