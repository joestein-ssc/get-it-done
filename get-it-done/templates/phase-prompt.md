...
> **Note:** Planning methodology is in `agents/gid-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/get-it-done/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/gid:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/get-it-done/references/tdd.md` for TDD plan structure.
...
See `~/.claude/get-it-done/templates/user-setup.md` for full schema and examples
...
See `~/.claude/get-it-done/workflows/verify-phase.md` for verification logic.
